...
export default function HighlightCard({ highlight, viewerPlayerId, onDeleted }: { highlight: any; viewerPlayerId: string | null; onDeleted: (id: string) => void }) {
  ...
  return (
    <a href={h.video_url} target="_blank" rel="noopener noreferrer" style={{
      ...
      border: '1px solid var(--border)', overflow: 'hidden', height: '100%', opacity: borrando ? 0.4 : 1,
    }}>
      <div style={{ position: 'relative', aspectRatio: '16/9', background: h.thumbnail_url ? 'var(--bg-surface)' : estilo.bg }}>
        ...
        <span style={{
          ...
          color: estilo.color, fontFamily: 'var(--font-mono)', fontSize: 9, fontWeight: 700,
          letterSpacing: '0.08em', textTransform: 'uppercase', padding: '3px 8px', border: `1px solid ${estilo.color}44`,
        ...
        <div style={{ position: 'absolute', top: 8, right: 8 }}>
          {esPropio ? (
            <button
              ...
              style={{ background: 'rgba(10,10,10,0.75)', border: '1px solid rgba(244,67,54,0.3)', width: 24, height: 24, cursor: borrando ? 'not-allowed' : 'pointer', color: 'rgba(244,67,54,0.7)', display: 'flex', alignItems: 'center', justifyContent: 'center' }}
      ...
      <div style={{ padding: '12px 14px', display: 'flex', flexDirection: 'column', flex: 1 }}>
        <div style={{
          fontFamily: 'var(--font-display-v2)', fontSize: 15, fontWeight: 600, color: 'var(--text-primary)',
        ...
        {h.torneo?.nombre && (
          <div style={{ fontSize: 11, color: 'var(--gold)', fontFamily: 'var(--font-mono)', marginBottom: 8, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>
            {h.torneo.nombre}
          </div>
        )}

        <div style={{ marginTop: 'auto', display: 'flex', alignItems: 'center', gap: 8, paddingTop: 8, borderTop: '1px solid var(--border)' }}>
          <div style={{ width: 22, height: 22, overflow: 'hidden', flexShrink: 0, background: 'var(--bg-input)', border: '1px solid var(--border)', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
            {h.jugador?.avatar_url
              ? <img src={h.jugador.avatar_url} alt="" style={{ width: '100%', height: '100%', objectFit: 'cover' }} />
              : <span style={{ fontSize: 10, fontFamily: 'var(--font-display-v2)', color: 'var(--gold)' }}>{(h.jugador?.discord_username ?? h.jugador?.nickname_juego ?? '?')[0]?.toUpperCase()}</span>}
          </div>
          <span style={{ fontSize: 12, color: 'var(--text-secondary)', flex: 1, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>
            {h.jugador?.discord_username ?? h.jugador?.nickname_juego ?? 'Jugador'}
          </span>
          <span style={{ fontFamily: 'var(--font-mono)', fontSize: 10, color: 'var(--text-muted)', flexShrink: 0 }}>{hace(h.created_at)}</span>
        </div>
      </div>
    </a>
  )
